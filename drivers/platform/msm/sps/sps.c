...
struct sps_bam *sps_h2bam(unsigned long h)
{
	...
	SPS_ERR("sps:Can't find BAM device for handle 0x%pK.", (void *)h);
...
int sps_connect(struct sps_pipe *h, struct sps_connect *connect)
{
	...
	bam = sps_h2bam(dev);
	if (bam == NULL) {
		SPS_ERR("sps:Invalid BAM device handle: 0x%pK",
					(void *)dev);
		result = SPS_ERROR;
		goto exit_err;
	}
...
int sps_disconnect(struct sps_pipe *h)
{
	...
	SPS_DBG2("sps:sps_disconnect: bam %pa src 0x%pK dest 0x%pK mode %s",
			BAM_ID(bam),
...
int sps_device_reset(unsigned long dev)
{
	...
	mutex_unlock(&bam->lock);
	if (result) {
		SPS_ERR("sps:Fail to reset BAM device: 0x%pK",
					(void *)dev);
		goto exit_err;
	}
